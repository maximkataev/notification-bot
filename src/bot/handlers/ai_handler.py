...
async def ai_del_command(message: types.Message, command: CommandObject):
    ...
    try:
        ...
        await message.reply("ID должен быть числом.", disable_web_page_preview=True)
        return
    ...
    if deleted:
        await message.reply(
            f"✓ Правило #{rule_id} удалено.", disable_web_page_preview=True
        )
        logger.info(f"AI rule deleted: #{rule_id} for user {user_id}")
    else:
        await message.reply("Правило не найдено.", disable_web_page_preview=True)
...
async def ai_reset_command(message: types.Message):
    ...
    if not rules:
        await message.reply(
            "У вас нет правил для удаления.", disable_web_page_preview=True
        )
        return
