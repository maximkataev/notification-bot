...
async def main():
    ...
    if rates.get("btc_usd"):
        ...
        if btc_24h is not None and btc_30d is not None:
            ...
            print(f"  Change: {arrow_24h}{abs(btc_24h):.1f}% (24h) | {arrow_30d}{abs(btc_30d):.1f}% (30d)")
        else:
            print(f"  Change: N/A")
    ...
    if rates.get("eth_usd"):
        ...
        if eth_24h is not None and eth_30d is not None:
            ...
            print(f"  Change: {arrow_24h}{abs(eth_24h):.1f}% (24h) | {arrow_30d}{abs(eth_30d):.1f}% (30d)")
        else:
            print(f"  Change: N/A")
    ...
    if rates.get("usd_eur"):
        ...
        if eur_24h is not None and eur_30d is not None:
            ...
            print(f"  Change: {arrow_24h}{abs(eur_24h_display)}% (24h) | {arrow_30d}{abs(eur_30d_display)}% (30d)")
        else:
            print(f"  Change: NOT AVAILABLE (will use historical data from DB once available)")
    ...
    if rates.get("usd_rub"):
        ...
        if rub_24h is not None and rub_30d is not None:
            ...
            print(f"  Change: {arrow_24h}{abs(rub_24h):.1f}% (24h) | {arrow_30d}{abs(rub_30d):.1f}% (30d)")
        else:
            print(f"  Change: NOT AVAILABLE (will use historical data from DB once available)")

    # Check if EUR/RUB have changes
    has_eur_changes = rates.get("eur_change_24h") is not None and rates.get("eur_change_30d") is not None
    has_rub_changes = rates.get("rub_change_24h") is not None and rates.get("rub_change_30d") is not None
